...
int main(int argc, char * argv[]){
  ...
  MPI_Comm_size(MPI_COMM_WORLD, &p);
	...
	// Should do a random seed that's different for each processor.
	// Parse seed from command line and add its rank.
	...
	while (t < Tfinal) {
		...
		// Use a MAX reduction on the processors and make it so every processor has it.
...
/*
Draw an exponential random variable with mean 1/rate.
*/
double drawExp(double rate) {
	E = -log(uniform64()) / rate;
	return E;
}


/*
Do KMC locally on a single section.
*/
double* local_KMC(crystal_site* h, int L) {
	double t = 0;
	while (t < t_stop) {
		//numEvents = 0;
		dt = -log(uniform64()) / R_loc; //draw from Exp(R_local)
