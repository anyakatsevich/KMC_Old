...
double KMC_section(int section_num, crystal_site *h, double t_stop, double R_loc, double K, int L_loc, int rank, int itr_num);
...
void initialize_lattice(crystal_site* h, int L_loc, int rank, int L, double K, int P, double* R_locs, MPI_Comm comm) {
	...
	for (int i = 1; i < L_loc + 1; i++) {
		...
		double heightVal = 1;//L*sin(2 * PI* x / ((double)L));
...
double KMC_section(int section_num, crystal_site *h, double t_stop, double R_loc, double K, int L_loc, int rank, int itr_num) {
	printf("heights at beginning of section %d:\n", section_num);

	for(int i = 0; i < L_loc; i++)
		printf("%d ", h[i].height);
	printf("\n");
//	printf("rank = %d, section = %d, initial R_loc  = %f\n", rank, section_num, R_loc);
/*	if (section_num == 0) {
		double temp = 0;
			for(int i = 1; i < L_loc / 2; i++) {
				temp += 2*h[i].rate;
			}
			printf("rank = %d, section = %d, R_loc second one = %f\n", rank, section_num, temp);
	}
	else {
		double temp = 0;
			for(int i = L_loc / 2; i <= L_loc; i++) {
    ...
    // printf("about to draw site\n");
		...
		i = drawSite(h, R_loc, section_num, L_loc);
	//	if (section_num==0)
  //	printf("%d\n", i);
	 // if (rank==0)
    // printf("drew site\n");
      ...
      if ((itr_num == 13 || itr_num == 14) && section_num ==0){

			printf("%d %d\n", siteupdatelist[0], siteupdatelist[1]);
			//	printf("heights after first section:\n");
			for(int i = 0; i < L_loc+2; i++)
				printf("%d ", h[i].height);
			printf("\n");
			...
			///printf("processor %d: i2 = %d\n", rank, i2);
				if (i2 > 0 && i2 < L_loc + 1) {
					...
					if ((section_num == 0 && i2 <  L_loc/2) || (section_num == 1 && i2 >= L_loc/2)){
 						R_loc -= oldRate;
					//	if (R_loc < 0)
					//	  printf("BAD\n");
						R_loc += 2*h[i2].rate;
...
int drawSite(crystal_site* h, double R_loc, int section_num, int L_loc) {
	/*
	printf(" section = %d, R_loc first one = %f\n", section_num, R_loc);
	if (section_num == 0) {
		double temp = 0;
			for(int i = 1; i < L_loc / 2; i++) {
				temp += 2*h[i].rate;
			}
			printf("section = %d, R_loc second one = %f\n",section_num, temp);
	}
	else {
		double temp = 0;
			for(int i = L_loc / 2; i <= L_loc; i++) {
	...
	*/
	...
	while (z2<eta) {
		...
		/*if (i == 3201) {
printf("eta = %f, z2 = %f\n", eta, z2);
			printf(" section_num = %d, drawSite i = %d\n", section_num, i);
		}*/
		z2 += 2 * h[i].rate;
	}
...
int main(int argc, char * argv[]) {
	...
	double R_max1, R_max2, c, n, t_stop, K, Tfinal, cpuTime;
        clock_t start, end;
  ...
  printf("L=%d, K=%f, T=%e, c = %f\n", L, K, Tfinal, c);
	...
	double R_loc1 = R_locs[0];
 	double R_loc2 = R_locs[1];
  printf("Rank = %d: initial R_loc1 = %f\n", rank, R_loc1);
  char str_hInit[100];
  ...
  int num_itr = 0;
	//while (t < Tfinal && num_itr < 40) {
  while(num_itr < 20){


		MPI_Allreduce(&R_loc1, &R_max1, 1, MPI_DOUBLE, MPI_MAX, MPI_COMM_WORLD);
  ...
  R_loc1 = KMC_section(0, h, t_stop, R_loc1, K, L_loc, rank, num_itr);
		printf("heights after first section:\n");
	for(int i = 0; i < L_loc; i++)
		printf("%d ", h[i].height);
	printf("\n");
if (rank == 1)
	printf("Rank = %d:  R_loc1 after first section = %f\n", rank, R_loc1);
	t+= t_stop;
   ...
   R_loc2 -=2*h[L_loc].rate;
   R_loc2 -= 2*h[L_loc-1].rate;
   ...
   R_loc2 += 2*h[L_loc-1].rate;
	 ...
	 MPI_Allreduce(&R_loc2, &R_max2, 1, MPI_DOUBLE, MPI_MAX, MPI_COMM_WORLD);
	 MPI_Barrier(MPI_COMM_WORLD);
	 t_stop = n/R_max2;
	  MPI_Barrier(MPI_COMM_WORLD);


    R_loc2 =  KMC_section(1, h, t_stop, R_loc2, K, L_loc, rank, num_itr);



		printf("iter = %d, heights after second section:\n", num_itr);
	 	for(int i = 0; i < L_loc; i++)
	 		printf("%d ", h[i].height);
	 	printf("\n");
   ...
   // if (rank==0)
    ...
    R_loc1 -= 2*h[2].rate;
if (rank == 1)
  printf("Rank = %d:  R_loc1  after subtracting stuff = %f\n", rank, R_loc1);
    //MPI_Barrier(MPI_COMM_WORLD);
		//COMMUNICATE BETWEEN PROCESSORS
    ...
    R_loc1 += 2*h[2].rate;
		  ...
		  //printf("Rank = %d:  R_loc1 after adding stuff = %f\n", rank, R_loc1);


		if (rank == 1)
	  	printf("rank = %d, num_itr = %d, R_loc1 = %f (after adding stuff), new t = %f\n", rank, num_itr, R_loc1, t);
   // if (num_itr == 0)
